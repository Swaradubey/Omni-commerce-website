...
import { X, UserCog, AlertCircle, MapPin, Phone, User, Mail, Lock } from 'lucide-react';
import { Button } from '../ui/button';
import { Input } from '../ui/input';
import { Label } from '../ui/label';
import { Textarea } from '../ui/textarea';
import { storeManagersApi } from '../../api/storeManagers';
...
interface StoreManagerModalProps {
  ...
  onClose: () => void;
  onCreated: () => void;
  /** When super admin creates from the client directory, pass the Mongo client _id */
...
export function StoreManagerModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user?.role === 'super_admin' && !clientIdOverride?.trim()) {
      setError('Client is required — use Add store manager from a client row in the table.');
      return;
    }
    if (!validate()) return;
    ...
    try {
      const body = {
        ...
        password,
        ...(user?.role === 'super_admin' && clientIdOverride
          ? { clientId: clientIdOverride.trim() }
          : {}),
      };
      const res = await storeManagersApi.create(body);
      if (res.success) {
        onCreated();
        onClose();
      } else {
        throw new Error(res.message || 'Could not add store manager');
      }
    } catch (err: unknown) {
        ...
        >
          <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-white/5 dark:bg-white/[0.02]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Add store manager</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Creates a dashboard login (role store manager) linked to this client
                </p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <Label htmlFor="sm-name" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <div className="space-y-2">
                <Label htmlFor="sm-email" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <div className="space-y-2">
                <Label htmlFor="sm-phone" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <div className="space-y-2">
                <Label htmlFor="sm-address" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="sm-password" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                ...
                <div className="space-y-2">
                  <Label htmlFor="sm-confirm" className="text-sm font-medium text-slate-700 dark:text-slate-200">
                    Confirm password
                  </Label>
                  <Input
                    id="sm-confirm"
                  ...
                  {fieldErrors.confirmPassword && (
                    <p className="text-xs font-medium text-red-600 dark:text-red-400">{fieldErrors.confirmPassword}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-3">
              ...
              <Button
                type="submit"
                disabled={isSubmitting}
                className="h-11 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 font-semibold text-white shadow-md shadow-indigo-500/25"
              >
                {isSubmitting ? 'Saving…' : 'Save store manager'}
              </Button>
            </div>
          </form>
        </motion.div>
